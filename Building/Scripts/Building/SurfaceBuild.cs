...
public class SurfaceBuild : BuilderTool
{
    ...
    {
        if (!Builder.Raycast(QueryTriggerInteraction.Collide, out position, out Vector3 normal, out var o))
        {
            rotation = Quaternion.identity;
            return false;
        }
        rotation = Quaternion.LookRotation(normal, Builder.Forward()) * Quaternion.Euler(new Vector3(-90, 180, 0));
